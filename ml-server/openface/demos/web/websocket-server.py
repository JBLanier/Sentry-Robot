...
class OpenFaceServerProtocol(WebSocketServerProtocol):
    ...
    def onMessage(self, payload, isBinary):
        ...
        elif msg['type'] == 'NEW_TARGET':
            self.newTarget(msg['val'])
        else:
    ...
    def move(self, up, right):
        self.moving = True
        robot.emit('move',{'up':up,'right':right})
    ...
    def processFrame(self, dataURL, identity):
        ...
        if not self.training:
            ...
            if (target != (-1,-1)):
                image_center = tuple([x/2 for x in annotatedFrame.shape[:2][::-1]])
                cv2.circle(annotatedFrame, center=image_center, radius=3,
                                   color=(50, 255, 0), thickness=-1)
                cv2.circle(annotatedFrame, center=target, radius=3,
                                   color=(255, 0, 0), thickness=-1)
                threshold = 40
                diff = tuple(np.subtract(target,image_center))
                print(diff)
                up = 0
                right = 0
                amount = 0.8
                ...
                if (abs(diff[0]) < threshold and abs(diff[1]) < threshold):
                    ...
                    if (diff[0] < -1 * threshold):
                        right = amount
                    if (diff[1] > threshold/4):
                        up = -1 * amount
                    if (diff[1] < -1 * threshold/4):
                        up = amount
